...
internal static class LibraryLoader
{
    ...
    public static IntPtr LoadLocalLibrary<T>(string libraryName)
    {
        ...
        static bool CheckLibraryPath(string root, string os, string arch, string libWithExt, out string foundPath)
        {
            if (!string.IsNullOrEmpty(root))
            {
                // a. in root
                string searchLib = Path.Combine(root, libWithExt);
    ...
    private static class Mac
    {
        ...
        public static IntPtr dlopen(string path, bool lazy = true) => dlopen(path, lazy ? RTLD_LAZY : RTLD_NOW);
